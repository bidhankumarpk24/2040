(() => {
  ...
  const $ = id => document.getElementById(id), boardEl = $('board'), scoreEl = $('score'), bestEl = $('best'), modal = $('modal');
  ...
  function setupModal() {
    ...
    const retry = dialog.querySelector('.modal-retry');
    if (retry) retry.onclick = newGame;
  ...
  function newGame() {
    ...
    score = 0; previous = null; won = false; ended = false;
    addTile(); addTile(); closeModal(); update();
  }
  function restore(state) {
    board = state.board.map(row => row.slice());
    score = state.score; won = state.won; ended = false;
    closeModal(); update();
  }
  function slide(line) {
    ...
    for (let i = 0; i < values.length; i++) {
      if (values[i] === values[i + 1]) { const value = values[i] * 2; result.push(value); gain += value; i++; }
  ...
  function move(direction) {
    ...
    if (direction === 'left' || direction === 'right') {
      for (let r = 0; r < SIZE; r++) {
        const original = board[r].slice(); let line = original.slice();
      ...
      for (let c = 0; c < SIZE; c++) {
        const original = board.map(row => row[c]); let line = original.slice();
  ...
  function beep(freq = 440) {
    if (!sound) return;
    try { const ctx = new (window.AudioContext || window.webkitAudioContext)(), osc = ctx.createOscillator(), gain = ctx.createGain(); osc.frequency.value = freq; gain.gain.value = .02; osc.connect(gain); gain.connect(ctx.destination); osc.start(); osc.stop(ctx.currentTime + .06); } catch (_) {}
  }
  function share() {
